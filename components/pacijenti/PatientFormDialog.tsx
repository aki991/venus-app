...
import type { PatientRecord } from "@/lib/db/admin";
...
function defaultsFrom(patient: PatientRecord | null): PatientFormValues {
  return {
...
export function PatientFormDialog({
  ...
  const {
    ...
    // U editu je broj kartona obavezan; u kreiranju je opcioni (auto ako prazno).
    resolver: zodResolver(editing ? patientEditFormSchema : patientFormSchema),
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    startTransition(async () => {
      if (editing && patient) {
        ...
        onOpenChange(false);
        router.push(`/pacijenti/${res.patientId}`);
      }
    });
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[560px]">
        <DialogHeader>
          <DialogTitle>
            {editing ? "Izmeni pacijenta" : "Novi pacijent"}
          </DialogTitle>
          <DialogDescription>
            {editing
        ...
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                placeholder={editing ? "" : "automatski ako prazno"}
              />
              {errors.card_number && (
                <p className="text-xs text-venus-danger">
          ...
          <div className="grid gap-1.5">
            ...
            <Textarea
              {...register("notes")}
              placeholder="Opšta napomena (NE medicinski podaci)"
            />
          </div>
